...
import StudentTopSellingItem from './StudentTopSellingItem';
import './studenttopSelling.css';

function StudentTopSelling() {
      ...
      <div className="card-body pb-0">
        ...
        <table className="table table-borderless">
          ...
          <tbody>
            {items &&
              items.length > 0 &&
              items.map(item => <StudentTopSellingItem key={item._id} item={item} />)}
          </tbody>
        </table>
      </div>
    </div>
...
export default StudentTopSelling;
